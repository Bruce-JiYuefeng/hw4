...
void AVLTree<Key, Value>::rotateLeft(AVLNode<Key, Value> *n)
{
    ...
    if (oldParent) {
        if (oldParent->getLeft() == n) oldParent->setLeft(c);
        else oldParent->setRight(c);
    } else {
        this->root_ = c;
    }
...
{
    AVLNode<Key, Value> *c = n->getLeft(); // Child
    AVLNode<Key, Value> *gc = c->getRight(); // Grandchild
